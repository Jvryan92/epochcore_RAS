...
# Define capsules for each stack
declare -A BASE_CAPSULES=(
    ["EpochCore Capsule Stock Market"]="SaaS capsules as equity"
...
declare -A IAAS_CAPSULES=(
    ["Cloud Resource Market"]="VM/storage as capsule equity"
...
declare -A PAAS_CAPSULES=(
    ["Dev Platform Marketplace"]="Capsule shares of frameworks"
...
declare -A SAAS_CAPSULES=(
    ["App Capsule Market"]="Equity in SaaS apps"
...
declare -A RAS_CAPSULES=(
    ["Agent Capsule Market"]="Equity in recursive agents"
...
# Mint function for a specific stack
mint_capsules() {
    local stack="$1"
    local capsule_array_name="${stack^^}_CAPSULES"
    local idx=0
    
    # Handle "base" stack which has different variable name
    if [[ "$stack" == "base" ]]; then
        capsule_array_name="BASE_CAPSULES"
    fi
    
    # Check if array exists
    if ! declare -p "$capsule_array_name" &>/dev/null; then
        echo "Error: No capsules defined for stack '$stack'"
        return 1
    fi
    
    # Convert array name to reference
    local -n capsule_ref="$capsule_array_name"
    local total_count=${#capsule_ref[@]}
    
    echo -e "\n🔄 Minting $total_count $stack capsules..."
    
    for title in "${!capsule_ref[@]}"; do
        ((idx++))
        local desc="${capsule_ref[$title]}"
        local id="CAP-${stack^^}-$(printf '%03d' $idx)"
        local ts=$(now)
        ...
        # Register with mesh trigger system
        local trigger_id="multistack_${stack}_${idx}"
        register_mesh_trigger "$stack" "$trigger_id" "$title - $desc"
        
        # Create mesh seal for trigger
        local mesh_seal=$(python3 -c "
import sys
import json
sys.path.append('/workspaces/epochcore_RAS/scripts')
try:
    ...
    seal = mesh.create_trigger_seal(
        '$trigger_id',
        {'capsule_id': '$id', 'stack': '$stack', 'timestamp': '$ts'}
    )
...
" 2>/dev/null || echo "{\"hash\": \"local-seal-$(date +%s)\", \"note\": \"Fallback seal created\"}")
        local seal_hash=$(echo "$mesh_seal" | jq -r '.hash' 2>/dev/null || echo "local-seal-$(date +%s)")
        ...
        local h=$(sha "$body")
        local capsule=$(echo "$body" | jq --arg h "$h" '. + {self_sha256:$h}')

        # Generate unique filename with timestamp and UUID
        local uuid=$(python3 -c "import uuid; print(uuid.uuid4())")
        local filename="capsule_$(date +%Y%m%d_%H%M%S)_${uuid}.json"
        local capsule_path="$OUTDIR/$filename"
        
        # Write capsule file
        echo "$capsule" > "$capsule_path"
        
        # Create archive path
        local archive_path="${capsule_path/multistack_capsules/archive}"
...
" 2>/dev/null || echo "Mesh trigger activation not available, continuing..."
        ...
        prev="$h"
        echo "✅ Minted $id — $title (Stack: ${stack})"
    done
    
    echo -e "✅ Completed minting $stack stack capsules!\n"
}

# Dispatch based on requested stack
if [[ "$STACK" == "all" ]]; then
    for s in base iaas paas saas ras; do 
        mint_capsules "$s" || echo "Warning: Error minting $s capsules"
    done
else
    mint_capsules "$STACK" || echo "Warning: Error minting $STACK capsules"
fi
