...
def main():
    ...
    execute_parser.add_argument("--validators", nargs="+", default=["validator1", "validator2", "validator3"], help="Validator nodes")
    ...
    if args.command == "create":
        ...
        print(f"Created cycle: {cycle['cycle_id']} with {len(cycle['task_assignments'])} tasks")
    
    elif args.command == "execute":
        result = executor.execute_full_cycle(args.cycle_id, args.validators, simulation=not args.real)
        ...
        if args.cycle_id in cycles["cycles"]:
            ...
            print(f"Tasks: {cycle['execution_metrics']['tasks_completed']}/{cycle['execution_metrics']['total_tasks']}")
        else:
            print(f"Cycle {args.cycle_id} not found")
    
    elif args.command == "sla":
        sla_status = executor.check_sla_compliance(args.cycle_id)
        print(f"SLA Compliance for {args.cycle_id}: {'PASS' if sla_status['compliant'] else 'FAIL'}")
        ...
        for cycle_id, cycle in cycles['cycles'].items():
            print(f"  {cycle_id}: {cycle['status']} (budget: {cycle['spent_budget']:.2f}/{cycle['budget']:.2f})")
