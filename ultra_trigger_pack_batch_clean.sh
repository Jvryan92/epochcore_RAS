#!/usr/bin/env bash
# ultra_trigger_pack_batch_clean.sh — Clean copy to resolve hidden formatting issues
set -euo pipefail
...
forge_one() {
  ...
  echo "{" > "$CAP"
  echo "  \"version\": \"1.0\"," >> "$CAP"
  echo "  \"capsule_id\": \"$BASE\"," >> "$CAP"
  echo "  \"trigger\": \"$TRIG\"," >> "$CAP"
  echo "  \"timestamp\": \"$TS\"," >> "$CAP"
  echo "  \"provenance\": {\"founder_note\": \"$PROV_NOTE\", \"true_north\": \"locked\"}," >> "$CAP"
  echo "  \"mesh\": {" >> "$CAP"
  echo "    \"monetary\": [\"Stripe\", \"MeshCredit\", \"ROI Glyphs\"]," >> "$CAP"
  echo "    \"governance\": [\"Multisig\", \"PR-vote\", \"Rollback-seal\"]," >> "$CAP"
  echo "    \"expansion\": [\"MeshSpawn\", \"CivilizationBlock\", \"Compound\"]" >> "$CAP"
  echo "  }," >> "$CAP"
  echo "  \"actions\": [\"Timestamp\", \"Log\", \"Seal\", \"Archive\", \"Reinject\"]," >> "$CAP"
  echo "  \"intent\": \"Forge ultra capsule for ROI + Mesh + Governance\"" >> "$CAP"
  echo "}" >> "$CAP"
