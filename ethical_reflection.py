...
class EthicalReflectionEngine:
    ...
    def __init__(self, reflection_dir: str = ".ethical/reflection"):
        ...
        self.ethical_insights = {
            ...
            "adaptation_metrics": {"current_rate": 0.0, "uncertainty_awareness": 0.0},
    ...
    def reflect_on_decision(
        ...
        principle_performance = {
            p: outcomes["principle_scores"].get(p, 0.0) for p in principles
        }
        ...
        suggestions = self._generate_suggestions(
            principle_performance, outcomes["overall_score"], context
        )

        # Update adaptation metrics
        adaptation_metrics = self._update_adaptation_metrics(
            confidence, outcomes["overall_score"]
        )
    ...
    def get_decision_support(
        ...
        principle_effectiveness = self._analyze_principle_effectiveness(
            similar_cases, proposed_principles
        )
        ...
        recommendations = {
            "confidence": (
                float(np.mean(confidence_values)) if confidence_values else 0.5
            ),
            "suggested_principles": [
                p for p, score in principle_effectiveness.items() if score > 0.7
            ],
            "cautions": [
    ...
    def _update_adaptation_metrics(
        self, confidence: float, overall_score: float
    ) -> Dict[str, float]:
        """Update adaptation and learning metrics"""
        if not self.reflection_history:
            return {"current_rate": 0.5, "uncertainty_awareness": 0.5}

        # Calculate learning rate from recent decisions
        recent_scores = [
            r["outcomes"]["overall_score"] for r in self.reflection_history[-5:]
        ]
    ...
    def _analyze_principle_effectiveness(
        self, cases: List[Dict[str, Any]], principles: List[str]
    ) -> Dict[str, float]:
    ...
    def _update_insights(self, reflection_data: Dict[str, Any]):
        ...
        confidences = [
            r["reflection"]["confidence"] for r in self.reflection_history[-10:]
        ]
        self.ethical_insights["confidence_trend"] = (
            np.mean(np.diff(confidences)) if len(confidences) > 1 else 0.0
        )
        ...
        self.ethical_insights["recent_lessons"] = reflection_data["reflection"][
            "lessons_learned"
        ]
