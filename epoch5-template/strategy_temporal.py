...
class TemporalProcessor:
    ...
    def add_time_series(self, 
                       ...
                       metadata: Optional[Dict[str, Any]] = None) -> TimeSeriesData:
    ...
    def forecast(self,
                series_name: str,
                horizon: int,
                num_samples: int = 100) -> TemporalPrediction:
        ...
        x = x.view(-1, batch_size, self.d_model)  # Shape: (seq_len, batch_size, d_model)
        ...
        conf_intervals_orig = confidence_intervals * self.stds[series_name] + self.means[series_name]
    ...
    def discover_causal_relations(self, 
                                series_names: List[str],
                                max_lag: int = 10,
                                significance_threshold: float = 0.05) -> List[CausalRelation]:
    ...
    def detect_temporal_patterns(self, 
                               series_name: str,
                               pattern_length: int = 24) -> Dict[str, Any]:
    ...
    def simulate_future_state(self,
        ...
        return {
            var: np.array(values)
            for var, values in state_history.items()
        }
    ...
    def _granger_causality(self,
        ...
        # Prepare lagged data
        X = np.column_stack([
            np.roll(cause, i) for i in range(lag)
        ])[lag:, :]
        ...
        f_stat = ((rss_restricted - rss_unrestricted) / lag) / (rss_unrestricted / (n - lag))
    ...
    def _detect_seasonality(self, values: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "strongest_period": 1 / dominant_freqs[0] if len(dominant_freqs) > 0 else None
        }
