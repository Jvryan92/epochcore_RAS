...
class ComprehensiveInvestmentAnalyzer:
    ...
    def generate_detailed_comparison(
            ...
            models[risk_level.name] = {
                ...
                "metrics": {
                    ...
                    "sharpe_ratio": self._calculate_sharpe_ratio(
                        params["return_rate"], params["volatility"]
                    ),
                },
                "simulation_results": {
                    ...
                    "success_rate": float(np.mean(scenarios > float(initial_capital))),
                },
            }
        ...
        return {
            ...
            "comparison_metrics": {
                "risk_adjusted_returns": {
                    level.name: self._calculate_risk_adjusted_return(
                        level.value["return_rate"], level.value["volatility"]
                    )
            ...
            "recommendations": self._generate_recommendations(
                initial_capital, monthly_contribution, time_horizon
            ),
        }
    ...
    def create_personal_plan(
        ...
        retirement_analysis = self._analyze_retirement_needs(
            retirement_goals, initial_capital, monthly_contribution, risk_tolerance
        )

        # Optimize tax strategy
        tax_strategy = self._optimize_tax_strategy(
            tax_params, retirement_analysis["required_savings"]
        )
        ...
        return {
            ...
            "action_items": self._generate_action_items(
                retirement_analysis, tax_strategy, timeline
            ),
    ...
    def optimize_tax_strategy(
        self, params: TaxParameters, target_savings: Decimal
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "tax_savings": self._calculate_tax_savings(tax_optimal_allocation, params),
            "recommendations": [
    ...
    def _run_monte_carlo(
        ...
        for i in range(simulations):
            ...
            for _ in range(time_horizon):
                ...
                annual_return = np.random.normal(float(return_rate), float(volatility))
    ...
    def _calculate_risk_adjusted_return(
        self, return_rate: Decimal, volatility: Decimal
    ) -> float:
    ...
    def _generate_recommendations(
        self, initial_capital: Decimal, monthly_contribution: Decimal, time_horizon: int
    ) -> List[str]:
    ...
    def _calculate_required_savings(
        ...
        # PMT = FV * r / ((1 + r)^n - 1)
        pmt = additional_needed * monthly_rate / ((1 + monthly_rate) ** months - 1)

        return pmt.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)

    def _create_monitoring_plan(self, risk_tolerance: RiskTolerance) -> Dict[str, Any]:
    ...
    def _generate_action_items(
        ...
        for account, amount in tax_strategy["optimal_allocation"].items():
            actions.append(f"Contribute ${float(amount):.2f} to {account}")
