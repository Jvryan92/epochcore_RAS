...
class InvestmentVisualizer:
    ...
    def _prepare_data(self, recommendations: List[Dict]) -> pd.DataFrame:
        ...
        df["initial_investment"] = df["initial_investment"].apply(lambda x: float(x))
        df["expected_outcome"] = df["expected_outcome"].apply(lambda x: float(x))

        # Extract growth rate percentage
        df["growth_rate"] = df["growth_rate"].apply(lambda x: float(x.strip("%")))
    ...
    def create_growth_comparison(self, recommendations: List[Dict]) -> str:
        ...
        ax1.bar(
            df["level"], df["initial_investment"], label="Initial Investment", alpha=0.6
        )
        ax1.bar(
            df["level"], df["expected_outcome"], label="Expected Outcome", alpha=0.6
        )
        ...
        # 2. Growth Rates
        ax2.plot(df["level"], df["growth_rate"], marker="o", linewidth=2, markersize=8)
        ...
        ax4.plot(df["time_horizon"], df["expected_outcome"], marker="s", linewidth=2)
