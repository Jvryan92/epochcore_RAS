...
class RiskAssessmentTool:
    ...
    def assess_risk_tolerance(self, responses: Dict[int, int]) -> Dict[str, Any]:
        ...
        for q_idx, r_idx in responses.items():
            ...
            if not 0 <= r_idx < len(question.options):
                raise ValueError(f"Invalid response index {r_idx} for question {q_idx}")
        ...
        recommendations = self._generate_recommendations(risk_level, factor_scores)

        return {
            "risk_score": risk_score,
            "risk_level": risk_level,
            "factor_analysis": {
                factor.value: score for factor, score in factor_scores.items()
            },
    ...
    def _generate_recommendations(
        self, risk_level: str, factor_scores: Dict[RiskFactor, float]
    ) -> List[str]:
        ...
        if income_score < 0.5:
            ...
            recommendations.append("Consider more liquid investments")
        ...
        if loss_score < 0.5:
            recommendations.append("Consider portfolio insurance strategies")
            recommendations.append("Focus on lower volatility investments")
        ...
        return [
            {
                ...
                "options": [
                    {"index": j, "text": opt["text"]} for j, opt in enumerate(q.options)
                ],
            }
