...
class SynthAgent(BaseAgent):
        ...
        for msg in metric_messages:
            ...
            for metric_name, value in metrics.items():
                ...
                if len(self.market_metrics[metric_name]) > max_history:
                    self.market_metrics[metric_name] = self.market_metrics[metric_name][
                        -max_history:
                    ]
    ...
    def _broadcast_market_alert(self, insights: Dict[str, Any]) -> None:
        ...
        self.send_message(None, "market.synthesis", alert, priority="high")
                    ...
                    recommendations.append(
                        {
                            ...
                            "direction": (
                                "increase" if change["change"] > 0 else "decrease"
                            ),
                        }
                    )
    ...
    def _forecast_values(self, values: List[float], horizon: int) -> List[float]:
