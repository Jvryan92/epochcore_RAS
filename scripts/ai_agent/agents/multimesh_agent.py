...
class MultiMeshAgent(AsyncBaseAgent):
    ...
    async def initialize_meshes(self) -> Dict[str, Any]:
        ...
        for mesh_name, mesh_config in self.mesh_configs.items():
            try:
                results[mesh_name] = await self._initialize_mesh(mesh_name, mesh_config)
    ...
    async def run_async(self) -> Dict[str, Any]:
        ...
        return {"meshes": results, "hypermesh": hyper_result, "status": "completed"}
    ...
    async def _initialize_mesh(self, name: str, config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._write_json(self.base_dir / f"{name}_chain_state.json", state)

            # Initialize mesh components
            components = {
                "ontology": {"ts": self._get_timestamp(), "graph": config["edges"]},
                "registry": {"ts": self._get_timestamp(), "agents": self.agents},
            ...
            for comp_name, comp_data in components.items():
                await self._write_json(
                    self.base_dir / f"{name}_{comp_name}.json", comp_data
                )
