                ...
                if active_syncs:
                    for sync in active_syncs:
                        state_icon = (
                            ...
                            else "🟡" if sync["state"] == "syncing" else "🔴"
                        )
                        print(
                            f"{state_icon} {sync['id']}: {sync['state']} "
...
async def main():
    ...
    parser.add_argument(
        "--monitor", action="store_true", help="Monitor sync points in real-time"
    )
    parser.add_argument(
        "--monitor-duration",
        type=int,
    ...
    parser.add_argument(
        "--diagnose", type=str, help="Diagnose specific sync point by ID"
    )
    parser.add_argument(
        "--list", action="store_true", help="List all active sync points"
    )
    parser.add_argument(
        "--cleanup", action="store_true", help="Clean up completed sync points"
    )
    parser.add_argument(
        "--stats", action="store_true", help="Show synchronization statistics"
    )
    parser.add_argument(
        "--registry", action="store_true", help="Show agent registry status"
    )
