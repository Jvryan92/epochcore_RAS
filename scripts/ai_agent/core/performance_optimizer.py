...
class PerformanceOptimizer(BaseAgent):
    ...
    def record_parameters(self, component: str,
                         params: Dict[str, Any],
        ...
        if any(self._is_metric_significant(component, metric, value)
        ...
        for product_id in product_ids:
            ...
            if component in self.parameter_history:
                ...
                if history and recent_metrics:
                    ...
                    if (recent_metrics.get('response_time', 0) > 800 or
                        recent_metrics.get('error_rate', 0) > 0.02):
            ...
            if len(history) > 1:
                ...
                for metric, value in latest['metrics'].items():
                    if metric in first_metrics:
                        pct_change = ((value - first_metrics[metric]) 
                                    / first_metrics[metric] * 100)
    ...
    def _get_compound_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for comp1, history1 in self.parameter_history.items():
            ...
            for comp2, history2 in self.parameter_history.items():
                ...
                if ('response_time' in metrics1 and 'response_time' in metrics2 and 
                    metrics1['response_time'] > 500 and metrics2['response_time'] > 500):
                ...
                if ('cpu_usage' in metrics1 and 'cpu_usage' in metrics2 and 
                    metrics1['cpu_usage'] > 0.7 and metrics2['cpu_usage'] > 0.7):
                    recommendations.append({
                        'components': [comp1, comp2],
                        'metric': 'cpu_usage',
                        'priority': 'high',
    ...
    def suggest_optimization(self, component: str,
                           current_params: Dict[str, Any],
                           target_metric: str,
                           min_confidence: float = 0.1) -> Tuple[Dict[str, Any], float]:
        ...
        best_params, confidence = self._generate_optimization(
            current_params, impacts
        )
    ...
    def _generate_optimization(self,
                             ...
                             impacts: Dict[str, Dict[str, float]]) -> Tuple[Dict[str, Any], float]:
        ...
        for param, values in impacts.items():
            ...
            best_value = min(values.items(), key=lambda x: x[1])[0]  # Use min for latency
    ...
    def apply_optimization(self, component: str,
                         old_params: Dict,
                         new_params: Dict,
                         improvement: float,
        ...
        proof = {
            ...
            'old_hash': hashlib.sha256(
                str(old_params).encode()
            ).hexdigest(),
            'new_hash': hashlib.sha256(
                str(new_params).encode()
            ).hexdigest(),
            'improvement': improvement,
            'confidence': confidence
        }
