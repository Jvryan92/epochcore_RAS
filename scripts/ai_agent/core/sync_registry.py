...
class SynchronizedAgentRegistry:
    ...
    async def save_registry(self, registry: Dict[str, AgentInfo]):
        ...
        async with self._lock:
            try:
                ...
                data = {
                    "agents": {
                        agent_id: info.to_dict() for agent_id, info in registry.items()
                    },
    ...
    async def update_agent_status(
        self, agent_id: str, status: AgentStatus, action: str = "status_update"
    ) -> bool:
    ...
    async def get_agents_with_sync_capability(self, capability: str) -> List[str]:
        ...
        try:
            ...
            return [
                agent_id
                for agent_id, info in registry.items()
                if capability in info.sync_capabilities
                and info.status == AgentStatus.ACTIVE
            ]
        except Exception as e:
