...
class PatternRecognizer:
    ...
    def _match_sequence(self, sequence: List[Dict], 
                       pattern: List[Dict]) -> bool:
    ...
    def discover_patterns(self, min_freq: int = 3,
                         min_conf: float = 0.7) -> List[Pattern]:
        """
        ...
        for i, mesh_id in enumerate(mesh_ids):
            ...
            for other_id in mesh_ids[i+1:]:
                ...
                common = self._find_common_sequences(
                    base_seq, other_seq
                )
    ...
    def _find_common_sequences(self, seq1: List[Dict], 
                             seq2: List[Dict]) -> List[Tuple[List[Dict], int, float]]:
        ...
        for size in range(2, min(len(seq1), len(seq2)) + 1):
            ...
            for w1 in windows1:
                matches = sum(1 for w2 in windows2 
                            if self._match_sequence(w2, w1))
                if matches > 0:
