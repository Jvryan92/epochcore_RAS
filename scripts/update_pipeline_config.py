...
class PipelineConfigOptimizer:
    ...
    def _apply_job_parallelization(
        self, config: Dict[str, Any], optimizations: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Apply job parallelization optimizations"""
        parallel_jobs = optimizations.get("job_parallelization", {}).get(
            "parallel_jobs", []
        )
        matrix_configs = optimizations.get("job_parallelization", {}).get(
            "matrix_configs", {}
        )
            ...
            if job_name in config["jobs"]:
                ...
                if job_name in cache_updates.get("cache_keys", {}):
                    ...
                    for step in cache_steps:
                        ...
                        step["with"]["restore-keys"] = cache_updates["cache_keys"][
                            job_name
                        ][1:]

        return config

    def _apply_resource_allocation(
        self, config: Dict[str, Any], optimizations: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Apply resource allocation optimizations"""
        resource_updates = optimizations.get("resource_allocation", {})

        for job_name, resource_class in resource_updates.get(
            "resource_classes", {}
        ).items():
            if job_name in config["jobs"]:
        ...
        for job_name, retry_config in test_updates.get("retry_config", {}).items():
            if job_name in config["jobs"]:
                ...
                for step in test_steps:
                    ...
                    retry_step["if"] = (
                        f"failure() && steps.{step['id']}.outcome == 'failure'"
                    )
