...
class DataCollectionAgent(SynchronizedAgent):
    ...
    async def collect_data(self):
        ...
        await self.registry.update_agent_status(
            self.name, AgentStatus.ACTIVE, "collecting_data"
        )
...
class ProcessingAgent(SynchronizedAgent):
    ...
    async def process_data(self, input_data):
        ...
        await self.registry.update_agent_status(
            self.name, AgentStatus.ACTIVE, "processing_data"
        )
...
class StorageAgent(SynchronizedAgent):
    ...
    async def store_data(self, input_data):
        ...
        await self.registry.update_agent_status(
            self.name, AgentStatus.ACTIVE, "storing_data"
        )
...
async def run_pipeline():
    ...
    # Create agents
    collectors = [
        DataCollectionAgent(f"collector_{i}", synchronizer, registry) for i in range(2)
    ]

    processors = [
        ProcessingAgent(f"processor_{i}", synchronizer, registry) for i in range(3)
    ]
    ...
    try:
        ...
        print(f"\n🎉 Pipeline completed successfully!")
        print(
            f"   📊 Data collected: {sum(len(data) for data in collected_data)} items"
        )
        print(
            f"   ⚙️  Data processed: {sum(len(data) for data in processed_data)} items"
        )
