...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("mesh_credit_sync.log"), logging.StreamHandler()],
)
    ...
    for file_path, file_info in json_files.items():
        try:
            ...
            if "manifest.json" in file_path:
                ...
                concepts["mint_triggers"] = data.get("rules", {}).get(
                    "mint_triggers", []
                )
                concepts["burn_triggers"] = data.get("rules", {}).get(
                    "burn_triggers", []
                )
                ...
                concepts["wallet_format"] = data.get("wallet_format", {}).get(
                    "encryption"
                )
