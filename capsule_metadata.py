...
class CapsuleManager:
    ...
    def create_capsule(self, capsule_id: str, content: str, metadata: Dict[str, Any] = None,
        ...
        self.log_integrity_event(capsule_id, "CAPSULE_CREATED", {
    ...
    def split_content_to_blocks(self, content: str, block_size: int = 1024) -> List[str]:
    ...
    def load_capsule(self, capsule_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.log_integrity_event(capsule_id, "CAPSULE_LOAD_ERROR", {"error": str(e)})
            return None
    ...
    def get_capsule_content(self, capsule_id: str) -> Optional[str]:
        ...
        try:
            ...
            if self.sha256(content) != capsule["content_hash"]:
                self.log_integrity_event(capsule_id, "CONTENT_INTEGRITY_VIOLATION", {
            ...
            self.log_integrity_event(capsule_id, "CONTENT_READ_ERROR", {"error": str(e)})
            return None
    ...
    def verify_capsule_integrity(self, capsule_id: str) -> Dict[str, Any]:
        ...
        verification_result["content_hash_valid"] = actual_content_hash == capsule["content_hash"]
        ...
        verification_result["merkle_verification"]["root_valid"] = merkle_tree.root_hash == capsule["merkle_root"]
        ...
        verification_result["overall_valid"] = (
            ...
            verification_result["merkle_verification"]["root_valid"] and
            verification_result["merkle_verification"]["blocks_failed"] == 0
        )
        
        # Log verification result
        self.log_integrity_event(capsule_id, "INTEGRITY_VERIFICATION", verification_result)
        
        return verification_result
    
    def create_metadata_entry(self, entry_id: str, capsule_refs: List[str], 
    ...
    def create_archive(self, archive_id: str, capsule_ids: List[str], 
        ...
        try:
            ...
            self.log_integrity_event(archive_id, "ARCHIVE_CREATED", {
            ...
            self.log_integrity_event(archive_id, "ARCHIVE_CREATION_FAILED", {"error": str(e)})
            return archive_info
...
def main():
    ...
    if args.command == "create-capsule":
        ...
        print(f"Integrity verification for {args.capsule_id}: {'VALID' if result['overall_valid'] else 'INVALID'}")
        if not result["overall_valid"]:
            print(f"Content hash valid: {result['content_hash_valid']}")
            print(f"Merkle root valid: {result['merkle_verification']['root_valid']}")
            print(f"Blocks verified: {result['merkle_verification']['blocks_verified']}")
        ...
        archive = manager.create_archive(args.archive_id, args.capsule_ids, not args.no_metadata)
        print(f"Created archive: {archive['archive_id']}")
        print(f"Status: {archive['status']}")
        if archive['status'] == 'completed':
            print(f"Files: {archive['file_count']}, Size: {archive['total_size']} bytes")
        ...
        for archive in archives:
            print(f"  {archive['archive_id']}: {archive['status']} ({archive['file_count']} files)")
