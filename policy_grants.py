...
class PolicyManager:
    ...
    def create_policy(self, policy_id: str, policy_type: PolicyType, 
                     parameters: Dict[str, Any], description: str = "") -> Dict[str, Any]:
        ...
        policy = {
            ...
            "enforced_count": 0,
            "violation_count": 0,
            "hash": self.sha256(f"{policy_id}|{policy_type.value}|{json.dumps(parameters, sort_keys=True)}")
        }
        return policy
    ...
    def create_grant(self, grant_id: str, grantee_did: str, resource: str, 
                    actions: List[str], conditions: Dict[str, Any] = None, 
        ...
        grant = {
            ...
            "used_count": 0,
            "active": True,
            "hash": self.sha256(f"{grant_id}|{grantee_did}|{resource}|{','.join(actions)}")
        }
        return grant
    ...
    def check_multi_sig_policy(self, policy: Dict[str, Any], signatures: List[str]) -> bool:
    ...
    def check_rate_limit_policy(self, policy: Dict[str, Any], did: str, 
                               current_requests: int) -> bool:
        """Check if rate limit policy is satisfied"""
        max_requests = policy["parameters"]["max_requests_per_hour"]
        return current_requests <= max_requests
    
    def check_skill_required_policy(self, policy: Dict[str, Any], agent_skills: List[str]) -> bool:
    ...
    def check_trust_threshold_policy(self, policy: Dict[str, Any], 
                                   agent_reliability: float) -> bool:
    ...
    def evaluate_policy(self, policy_id: str, context: Dict[str, Any]) -> bool:
        ...
        try:
            if policy_type == PolicyType.QUORUM:
                ...
                result = self.check_multi_sig_policy(policy, context.get("signatures", []))
            elif policy_type == PolicyType.RATE_LIMIT:
                result = self.check_rate_limit_policy(policy, context.get("did", ""), 
                                                    context.get("current_requests", 0))
            elif policy_type == PolicyType.SKILL_REQUIRED:
                result = self.check_skill_required_policy(policy, context.get("agent_skills", []))
            elif policy_type == PolicyType.TRUST_THRESHOLD:
                result = self.check_trust_threshold_policy(policy, 
                                                         context.get("agent_reliability", 0.0))
            else:
                result = False
    ...
    def check_grant(self, grant_id: str, requester_did: str, resource: str, action: str) -> bool:
    ...
    def log_violation(self, policy_id: str, context: Dict[str, Any]):
        ...
        violation = {
            ...
            "policy_id": policy_id,
            "context": context,
            "hash": self.sha256(f"{self.timestamp()}|{policy_id}|{json.dumps(context, sort_keys=True)}")
        }
    ...
    def get_valid_grants(self, did: str) -> List[Dict[str, Any]]:
        ...
        for grant in grants["grants"].values():
            if (grant["active"] and 
                grant["grantee_did"] == did and
                now <= datetime.fromisoformat(grant["expires_at"].replace('Z', '+00:00'))):
...
def main():
    ...
    policy_parser.add_argument("policy_type", choices=["quorum", "multi_sig", "rate_limit", "skill_required", "trust_threshold"])
    ...
    if args.command == "create-policy":
        ...
        policy = manager.create_policy(args.policy_id, PolicyType(args.policy_type), parameters)
        manager.add_policy(policy)
        print(f"Created policy: {policy['policy_id']} ({policy['hash']})")
    
    elif args.command == "create-grant":
        grant = manager.create_grant(args.grant_id, args.grantee_did, args.resource, args.actions)
        ...
        for policy in policies:
            print(f"  {policy['policy_id']}: {policy['type']} (enforced: {policy['enforced_count']}, violations: {policy['violation_count']})")
        ...
        for grant in grants['grants'].values():
            ...
            print(f"  {grant['grant_id']}: {grant['grantee_did']} -> {grant['resource']} [{status}]")
