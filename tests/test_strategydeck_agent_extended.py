...
def agent():
    ...
    with patch("strategydeck_agent.CeilingManager") as mock_ceiling_manager, patch(
        "strategydeck_agent.EpochAudit"
    ) as mock_audit, patch("strategydeck_agent.ServiceTier") as mock_service_tier:
...
def test_get_ceiling_status_no_config(agent):
    ...
    agent.ceiling_manager.load_ceilings.return_value = {"configurations": {}}
...
def test_health_check_full(agent):
    ...
    with patch.object(agent, "get_ceiling_status") as mock_ceiling_status, patch.object(
        agent, "verify_security"
    ) as mock_verify_security:
...
def test_health_check_degraded(agent):
    ...
    with patch.object(agent, "get_ceiling_status") as mock_ceiling_status, patch.object(
        ...
        mock_ceiling_status.return_value = {"error": "Ceiling system unavailable"}
...
def test_health_check_security_degraded(agent):
    ...
    with patch.object(agent, "get_ceiling_status") as mock_ceiling_status, patch.object(
        ...
        mock_ceiling_status.return_value = {"config_id": "agent_testagent"}
...
def test_cli_run_command():
    ...
    with patch("argparse.ArgumentParser") as mock_parser, patch(
        "strategydeck_agent.StrategyDECKAgent"
    ) as mock_agent_class:
...
def test_cli_status_command():
    ...
    with patch("argparse.ArgumentParser") as mock_parser, patch(
        ...
        # Run the CLI code
        with patch("sys.argv", ["strategydeck_agent.py", "status"]), patch(
            "builtins.print"
        ) as mock_print, patch("json.dumps") as mock_dumps:
...
def test_cli_demo_mode():
    ...
    with patch("argparse.ArgumentParser") as mock_parser, patch(
        ...
        # Run the CLI code
        with patch("sys.argv", ["strategydeck_agent.py"]), patch(
            "builtins.print"
        ) as mock_print:
...
def test_error_handling_in_cli():
    ...
    with patch("argparse.ArgumentParser") as mock_parser, patch(
        ...
        # Run the CLI code
        with patch(
            "sys.argv", ["strategydeck_agent.py", "run", "--goal", "Test goal"]
