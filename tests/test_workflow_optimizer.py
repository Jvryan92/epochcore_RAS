...
class TestWorkflowOptimizerUnified:
    ...
    def test_workflow_analysis_unified(self):
        ...
        assert any("parallel" in s.lower() for s in suggestions)  # Should suggest parallelization
        assert any("resource" in s.lower() for s in suggestions)  # Should mention resource optimization
    ...
    def test_workflow_optimization_unified(self):
        ...
        assert all(task["id"] in resource_optimized["allocations"] 
                  for task in workflow_data["tasks"])
        assert resource_optimized["feasible"]  # Should be possible with given constraints
        ...
        assert all(resource in cost_optimized["cost_breakdown"] 
    ...
    def test_workflow_monitoring_unified(self):
        ...
        assert all(metric in report["resource_utilization"] 
