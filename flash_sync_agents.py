...
AGENTS = [
    {
        'id': 'agent://alpha',
        'did': generate_did(),
        'skills': ['scrape.web', 'vector.store', 'atomize.payload', 'cohere.frames', 'sandbox.dryrun'],
        'rel': 0.94,
        'lat': 210,
        'status': 'active'
    },
    {
        'id': 'agent://bravo',
        'did': generate_did(),
        'skills': ['plan.compose', 'review.policies', 'diffuse.channels', 'schedule.drip', 'rollback.diff'],
        'rel': 0.92,
        'lat': 250,
        'status': 'active'
    },
    {
        'id': 'agent://gamma',
        'did': generate_did(),
        'skills': ['echo.measure', 'blackboard.merge', 'snapshot.world', 'sign.proof', 'attest.supply'],
        'rel': 0.90,
        'lat': 300,
        'status': 'active'
    },
...
def flash_sync_agents():
    ...
    for agent in AGENTS:
        ...
        snapshot = {
            ...
            'skills_active': random.sample(agent['skills'], k=max(1, len(agent['skills'])-1)),
    ...
    if latencies:
        ...
        for i, snapshot in enumerate(snapshots):
            ...
            if abs(z_score) > 1.5:
                ...
                sync_receipt['consensus_achieved'] = sync_receipt['consensus_achieved'] and (
                    abs(z_score) < FLASH_THRESHOLD)
        ...
        'agents': [
            {
                ...
                'skills': agent['skills'],
                'status': 'active',
                'reliability': next((s['reliability'] for s in snapshots if s['agent_id'] == agent['id']), agent['rel']),
                'latency_ms': next((s['latency_ms'] for s in snapshots if s['agent_id'] == agent['id']), agent['lat']),
