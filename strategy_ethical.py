...
class EthicalEngine:
    ...
    def assess_action(self, 
                     ...
                     stakeholders: Optional[List[str]] = None) -> EthicalAssessment:
        ...
        reflection = self.reflection.reflect_on_decision(
            ...
            outcomes={
                ...
                "principle_scores": {p.value: s for p, s in zip(EthicalPrinciple, principle_scores)}
            }
        )
        ...
        decision_support = self.reflection.get_decision_support(
            context=context,
            proposed_principles=[p.value for p in EthicalPrinciple]
        )
        ...
        reasoning.extend([
            f"Improvement: {suggestion}" for suggestion in reflection.improvement_suggestions[:2]
        ])
    ...
    def predict_impact(self,
                      action_id: str,
                      context: Dict[str, Any],
                      time_horizon: str = "long_term") -> Impact:
    ...
    def get_ethical_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "constraint_satisfaction_rate": np.mean([
                1 if a.constraints_satisfied else 0
        ...
        metrics.update({
            "metacognitive_metrics": {
                ...
                "adaptation_rate": reflection_insights["adaptation_metrics"]["current_rate"],
                "uncertainty_awareness": reflection_insights["adaptation_metrics"]["uncertainty_awareness"]
            },
            "learning_progress": {
                "principle_effectiveness": reflection_insights["principle_effectiveness"],
    ...
    def _model_indirect_effects(self,
                              direct_effects: Dict[str, float],
        ...
        for effect, magnitude in direct_effects.items():
            for stakeholder in stakeholders:
                if stakeholder in self.stakeholder_registry:
                    sensitivity = self.stakeholder_registry[stakeholder]["impact_sensitivity"]
                    indirect_effects[f"{effect}_{stakeholder}"] = magnitude * sensitivity
        return indirect_effects
        
    def _project_long_term_effects(self,
                                 direct_effects: Dict[str, float],
                                 indirect_effects: Dict[str, float],
    ...
    def _calculate_uncertainty(self,
                             direct_effects: Dict[str, float],
                             indirect_effects: Dict[str, float],
                             long_term_effects: Dict[str, float]) -> float:
        """Calculate uncertainty in impact prediction"""
        # Uncertainty increases with effect chain length and magnitude
        direct_uncertainty = np.std(list(direct_effects.values())) if direct_effects else 0
        indirect_uncertainty = np.std(list(indirect_effects.values())) if indirect_effects else 0
        long_term_uncertainty = np.std(list(long_term_effects.values())) if long_term_effects else 0
        
        # Weight uncertainties by their temporal distance
        return (0.2 * direct_uncertainty +
    ...
    def _assess_reversibility(self,
        ...
        total_magnitude = (sum(map(abs, direct_effects.values())) +
