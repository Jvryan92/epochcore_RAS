...
class FeatureScaler:
    ...
    def __init__(self):
        self.scaling_methods = {
            ...
            ScalingMethod.MIN_MAX: MinMaxScaler,
            ScalingMethod.ROBUST: RobustScaler,
            ScalingMethod.UNIT_VECTOR: self._unit_vector_scaler
        }
...
class DimensionalityReducer:
    ...
    def __init__(self):
        self.reduction_methods = {
            ReductionMethod.PCA: self._apply_pca,
            ReductionMethod.TSNE: self._apply_tsne,
            ReductionMethod.RANDOM_PROJECTION: self._apply_random_projection
        }
    ...
    def _apply_pca(self, X: np.ndarray, n_components: int) -> Tuple[np.ndarray, PCA]:
    ...
    def _apply_tsne(self, X: np.ndarray, n_components: int) -> Tuple[np.ndarray, TSNE]:
        """Apply t-SNE dimensionality reduction"""
        n_components = min(n_components, 3)  # t-SNE typically used for 2D/3D
        # Use fewer samples for t-SNE to avoid memory issues
        if X.shape[0] > 1000:
        ...
        X_reduced_sample = tsne.fit_transform(X_sample)
        
        # For full dataset, use PCA as approximation
        if X.shape[0] > 1000:
            pca = PCA(n_components=n_components)
            X_reduced = pca.fit_transform(X)
            return X_reduced, tsne
        
        return X_reduced_sample, tsne
